...
# Title
st.title('🌎 Simplified LULC Change Detection and Future Prediction App')

# New Class Labels and Colors
class_label_mapping = {
    0: 'NoData',
    1: 'Water',
    2: 'Cropland',
...
class_color_mapping = {
    0: '#ffffff',  # White for NoData (optional, will mask later)
    1: 'blue',
    2: 'yellow',
    3: 'red',
    4: 'green',
...
def create_color_mask(image_array, classes_present):
    color_image = np.ones((image_array.shape[0], image_array.shape[1], 3), dtype=np.float32)  # Default white
...
def plot_map(image_array, title, transform, crs, classes_present, is_transition=False, transition_info=None):
    ...
    if is_transition and transition_info:
        ...
        labels = [class_label_mapping[c] for c in classes_present if c != 0]
        colors = [class_color_mapping[c] for c in classes_present if c != 0]
...
if uploaded_file_1 and uploaded_file_2:
    try:
        ...
        if lc1.shape != lc2.shape:
            ...
            lc1 = np.where((lc1 <= 0) | (lc1 >= 2147483647), np.nan, lc1)
            lc2 = np.where((lc2 <= 0) | (lc2 >= 2147483647), np.nan, lc2)
            valid_mask = (~np.isnan(lc1)) & (~np.isnan(lc2))

            # Plot Year 1
            classes_present_1 = sorted(list(set(np.unique(lc1[~np.isnan(lc1)]).astype(int))))
            st.subheader(f'🗺️ LULC Map for {year1}')
            plot_map(lc1, f'LULC Map for {year1}', transform1, crs1, classes_present_1)

            # Plot Year 2
            classes_present_2 = sorted(list(set(np.unique(lc2[~np.isnan(lc2)]).astype(int))))
            st.subheader(f'🗺️ LULC Map for {year2}')
            plot_map(lc2, f'LULC Map for {year2}', transform2, crs2, classes_present_2)

            # 📋 Pixel Count Table
            st.subheader('📋 Pixel Count Table (Year 1 vs Year 2)')
            pixel_count_table = pd.DataFrame({
                'Class': list(range(1, 6)),
                'Class Name': [class_label_mapping[i] for i in range(1, 6)],
                f'Pixels ({year1})': [np.nansum(lc1 == i) for i in range(1, 6)],
                f'Pixels ({year2})': [np.nansum(lc2 == i) for i in range(1, 6)]
            })
            st.dataframe(pixel_count_table)

            # 📋 Transition Matrix Table
            st.subheader('📋 Transition Matrix Table')
            transition_matrix = np.zeros((6, 6), dtype=int)
            for i in range(6):
                for j in range(6):
                    transition_matrix[i, j] = np.nansum((lc1 == i) & (lc2 == j))
            transition_df = pd.DataFrame(transition_matrix,
                                         index=[f'From {class_label_mapping[i]}' for i in range(6)],
                                         columns=[f'To {class_label_mapping[i]}' for i in range(6)])
            st.dataframe(transition_df)

            # Transition Map
            transition_map = lc1 * 10 + lc2
            ...
            for code in transitions_unique:
                from_class = code // 10
                to_class = code % 10
                transition_labels.append(f"{class_label_mapping.get(from_class, 'Unknown')} ➔ {class_label_mapping.get(to_class, 'Unknown')}")
            ...
            if future_steps > 0:
                ...
                plot_map(predicted_lulc, f'Predicted LULC Map for {future_year}', transform2, crs2,
                         classes_present=list(range(1,6)))
                ...
                predicted_pixel_table = pd.DataFrame({
                    'Class': list(range(1, 6)),
                    'Class Name': [class_label_mapping[i] for i in range(1, 6)],
                    f'Pixels ({future_year})': [np.nansum(predicted_lulc == i) for i in range(1, 6)]
                })
